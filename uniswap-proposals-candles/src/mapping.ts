...
export function handleInitialize(event: InitializeEvent): void {
    ...
    // pools are sparse, so that window is long and visible.
...
export function handleMint(event: MintEvent): void {
    ...
    if (pool) {
        // pool.liquidity = pool.liquidity.plus(event.params.amount) // Mint event params different in V3?
        // Uniswap V3 Mint event: (sender, owner, tickLower, tickUpper, amount, amount0, amount1)
        // amount is liquidity
...
export function handleBurn(event: BurnEvent): void {
    let pool = Pool.load(event.address.toHexString())
    if (pool) {
        // Uniswap V3 Burn event: (owner, tickLower, tickUpper, amount, amount0, amount1)
